...
function updateArmAngle() {
  ...
  */
  ...
  if (leftWrist.score > 0.3 && leftElbow.score > 0.3 && leftShoulder.score > 0.3) {
    ...
    document.getElementById("debug_4").innerHTML = "elbowAngle: " + "Cannot see elbow";
  }
  document.getElementById("debug_4").innerHTML = "elbowAngle: " + Math.round(elbowAngle / 10) * 10;
}
...
function updateBackAngle() {
  ...
  if ((backAngle < 20) || (backAngle > 160)) {
    highlightBack = false;
    document.getElementById("debug_3").innerHTML = "Back is straight";
  } else {
    highlightBack = true;
    ...
    document.getElementById("debug_3").innerHTML = "Back is not straight";
  }

}

function inUpPosition() {
  if (elbowAngle > (180 - 30) && elbowAngle < (180 + 30)) {
    document.getElementById("debug_1").innerHTML = "In up position";
...
function inDownPosition() {
  ...
  if (poses[0].keypoints[0].y > poses[0].keypoints[7].y) {
    elbowAboveNose = true;
    document.getElementById("debug_2").innerHTML = "Elbow is above nose";
  } else {
    document.getElementById("debug_2").innerHTML = "Elbow is below nose";
  }

  if ((highlightBack == false) && elbowAboveNose && ((abs(elbowAngle) > (90 - 30)) && (abs(elbowAngle) < (90 + 30)))) {
    document.getElementById("debug_1").innerHTML = "In down position";
    if (upPosition == true) {
